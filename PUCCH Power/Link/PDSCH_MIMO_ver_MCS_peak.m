...
cfg.DelayProfile = 'AWGN';    %'EVA';        % Delay profile
...
rmc.NDLRB = 100;
rmc.CFI = 1;
rmc.PDSCH.Modulation = {'64QAM','64QAM'};
rmc.PDSCH.PRBSet = [0:99]';
rmc.PDSCH.CodedTrBlkSizes = [0,0,0,0,86400,0,0,0,0,0;...
                             0,0,0,0,86400,0,0,0,0,0];
rmc.PDSCH.TrBlkSizes = [0,0,0,0,75376,0,0,0,0,0;...
                        0,0,0,0,75376,0,0,0,0,0];                     
...
%     harqprocess = hDownlinkSpatialMuxNewHARQProcess(rmc);
    ...
    for subframeNo = 0:(NFrames*10-1)
        ...
        figure();
        scatter(real(reshape(GRID,[],1)),imag(reshape(GRID,[],1)));
        grid on;
