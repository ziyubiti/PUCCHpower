...
%parfor index = 1:numel(SNRdB)
    ...
    for subframeNo = 0:(NFrames*10-1)
        ...
        txWaveform = [lteRMCDLTool(rmc, ...
                   {harqProcess(harqIdx).dlschTransportBlk}); zeros(25,p)];
...
%                 frc, harqProc(harqIdx).ulschTransportBlk);
        ...
        if (user==1)
            rxWaveform = lteFadingChannel(channel,[txWaveform;zeros(25,frc.NTxAnts)]);
        else
            rxWaveform = rxWaveform + ...
                lteFadingChannel(channel,[txWaveform*10^(usersPDSCHpower(user)/20); zeros(25,frc.NTxAnts)]);
        end;
        ...
        % bj: add repeater signal,delay and power different
        repeaterSeed = 6789;
        channel.Seed = repeaterSeed;
        % modify FD data, according TD data
        rxSubframe0 = lteOFDMDemodulate(rmc,txWaveform(1:ofdmInfo.SamplingRate/1000,:));
