...
rvSeq = 0; %[0 2 3 1];                     % the length is to constrait the trans num,
...
% frc.PUSCH.PRBSet = [1:48;1:48].';
frc.PUSCH.PRBSet = [0:24;0:24].';
% frc.PUSCH.PRBSet = [25:49;25:49].';
frc.PUSCH.TrBlkSizes = [2216,2216,2216,1320,2216,2216,2216,8504,1320,2216];
%  frc.PUSCH.TrBlkSizes = [46888,46888,46888,46888,46888,46888,46888,46888,46888,46888];
 frc.PUSCH.CodedTrBlkSizes = [57600,57600,7200,13824,57600,57600,57600,27648,13824,57600];
...
%     N = 1/(SNR*sqrt(double(info.Nfft)))/sqrt(2.0);   
    ...
    offsetused = 0;
    for subframeNo = 2 %0:numSubframes-1%(NFrames*10-1)
...
%         rxWaveform = txWaveform + noise(1:30720,1);
        ...
        if 1   
          dspdata = load('D:\2016 ver\0510 pdcch\ul0\UL_SF7.txt');
...
%
        ...
        if DebugFlag
            ...
            h = (ifft(estChannelGrid(drsindex,12)));
            figure();
            stem(abs(h));
            grid on;
